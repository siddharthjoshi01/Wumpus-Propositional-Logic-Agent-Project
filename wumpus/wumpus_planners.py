...
class PlanRouteProblem(search.Problem):
    ...
    def h(self,node):
        ...
        """
        "*** YOUR CODE HERE ***"
        goal_md = [manhattan_distance_with_heading(node.state, i) for i in self.goals]
        min_md = min(goal_md)
        return min_md
    ...
    def actions(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        head_f = ['Forward', 'TurnRight', 'TurnLeft']
        other = ['TurnLeft', 'TurnRight']
        x,y,heading = state[0],state[1],state[2]
        if heading==3 and (x+1,y) in self.allowed:
            return head_f
        elif heading==2 and (x,y-1) in self.allowed:
            return head_f
        elif heading==1 and (x-1,y) in self.allowed:
            return head_f
        elif heading==0 and (x,y+1) in self.allowed:
            return head_f
        else:
            return other
    ...
    def goal_test(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        all_states = state[0:2]
        return all_states in self.goals
...
class PlanShotProblem(search.Problem):
    ...
    def h(self,node):
        ...
        "*** YOUR CODE HERE ***"
        shot_xy = []
        allowed_xy = self.allowed
        possible_xy = self.goals
        i,j = 0,0
        while (i<possible_xy):
            while (j<allowed_xy):
                ...
                j+=1
            i+=1
        shot_xy_md = [manhattan_distance_with_heading(node.state, i) for i in shot_xy]
        shot_xy_mdmin = min(shot_xy_md)
        return shot_xy_mdmin
    ...
    def goal_test(self, state):
        ...
        x1,y1 = xy[0],xy[1]
        xy = 0,0
        while (xy < possible_xy):
            if (x1==x):
                if not (heading==0 and (y1>y)):
                    return False
                elif not (heading==2 and (y1<y)):
                    return False
                else:
                    pass
            elif (y1==y):
                if not (heading==1 and (x1<x)):
                    return False
                elif not (heading==3 and (x1>x)):
                    return False
                else:
                    pass
            else:
